...
| 2.03 | Validated Actions List | ADEXMO Recommended Path #15 | Required | All Actions validated by the human. Domains ownership verified. No technical Actions. No duplicates. Inputs and outputs clear. Rules and constraints attached. Each Action independent from interface. Stable enough to be used by implementation teams. `actions-list.md` exists in the project repository with status Validated. |
